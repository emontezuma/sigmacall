...
export class UsuariosComponent implements OnInit {
  ...
  filtrar()
  {
    ...
    this.totalRegistros = 'Hay ' + (this.registros.length == 1 ? 'un usuario' : this.registros.length + ' usuarios') + ' en la lista' + cadAdicional;  
  }
  ...
  llenarRegistros()
  {
    ...
    this.registros = [];
    this.arrFiltrado = [];
    let consulta = "SELECT IF(a.estatus = 'A', 'activo', 'inactivo') AS miestatus, a.id, a.nombre, IFNULL(a.modificacion, 'N/A') AS fecham, IFNULL(b.nombre, '') AS userm FROM cat_usuarios AS a LEFT JOIN cat_usuarios AS b ON a.modificado = b.id ORDER BY a.nombre";
    ...
    this.servicio.consultasBD(campos).subscribe((datos: any []) =>{
      if (datos)
      {
        ...
        setTimeout(() => {
          this.totalRegistros = 'Hay ' + (this.registros.length == 1 ? 'un usuario' : this.registros.length + ' usuarios') + ' en la lista';  
